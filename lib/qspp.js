...
// Sum training hours that fall within a given calendar year.
// records: array of { training_date, hours }; year: number e.g. 2026
export function hoursInYear(records, year) {
  return (records || [])
    .filter((r) => r.training_date && r.training_date.slice(0, 4) === String(year))
    .reduce((sum, r) => sum + Number(r.hours || 0), 0);
}

// The year the current cycle's progress "this year" should count against.
// Uses today's calendar year, clamped to the cycle window.
export function currentTrainingYear(cycle, today = new Date()) {
  if (!cycle) return today.getFullYear();
  const y = today.getFullYear();
  const startY = cycle.start.getFullYear();
  const endY = cycle.end.getFullYear();
  return Math.min(Math.max(y, startY), endY);
}
