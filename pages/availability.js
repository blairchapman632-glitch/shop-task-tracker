...
export default function AvailabilityPage() {
  ...
  const [allLeave, setAllLeave] = useState([]);       // everyone's pending+approved leave (for calendar shading)
  const [blackouts, setBlackouts] = useState([]);     // manager blackout periods
  const [leaveMonthOffset, setLeaveMonthOffset] = useState(0); // month shown in the leave calendar
...
const LEAVE_TYPES = ["Annual Leave", "Personal/Carer's Leave", "Unpaid Leave"];
  ...
  const handleSubmitLeave = async () => {
    ...
    try {
      ...
      const myRole = selectedStaff.role;
